...
class DeconzAPI extends EventEmitter {
  ...
  buildFrame(frame, encapsulate) {
    if (typeof encapsulate === 'undefined') {
      encapsulate = true;
    }
    ...
    const rawFrame = packet.slice(0, builder.length);
    if (encapsulate) {
      return this.encapsulateFrame(rawFrame);
    }
    return rawFrame;
  }

  encapsulateFrame(rawFrame) {
    // Encapsulate the packet as per SLIP protocol
    return Slip.Encapsulate(rawFrame);
  }
